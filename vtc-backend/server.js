...
// Convertit des coordonnées GPS (lat, lng) en adresse lisible.
// body: { lat: 43.7, lng: 7.25 }
app.post('/api/reverse-geocode', async (req, res) => {
  try {
    ...
    if (!result) {
      return res.status(422).json({ error: 'Impossible de déterminer votre adresse.' });
    }

    res.json({ address: result.formatted_address });
  } catch (err) {
    console.error('Erreur /api/reverse-geocode', err);
    res.status(500).json({ error: 'Erreur serveur lors de la géolocalisation.' });
  }
});
